...
public class NguoiDungValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		if (!isValid(user.getPassword())) {
			errors.rejectValue("password", "error.password", "Mật khẩu phải gồm ít nhất: 1 chữ hoa, 1 chữ số, 1 chữ thường, 1 kí tự đặc biệt và có độ dài từ 8-32");
		}
		...
		final Pattern phonePattern = Pattern.compile(PASSWORD_PATTERN);
	
		if(user.getSoDienThoai() != null){
			Matcher matcher = phonePattern.matcher(user.getSoDienThoai());
			if(!matcher.matches()){
				errors.rejectValue("soDienThoai", "error.soDienThoai", "Định dạng số điện thoại sai");
			}
		}
