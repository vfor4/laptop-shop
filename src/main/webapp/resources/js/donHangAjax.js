$(document).ready(function() {
				...
				$.each(result.content, function(i, donHang){
					...
					var donHangRow =  '<tr>' +
					     ...
					     if(donHang.trangThaiDonHang == "Đang chờ giao" || donHang.trangThaiDonHang == "Đang giao"){
					    	 donHangRow += ' &nbsp;<button class="btn btn-primary btnPhanCong">Tạo đơn</button>'+
    ...
    // event khi click vào button 
	$(document).on('click', '.btnPhanCong', function (event){
		event.preventDefault();
		var donHangId = $(this).parent().prev().children().val();
		createOrder(donHangId)
		// $("#donHangId").val(donHangId);
		// console.log(donHangId);
		// $("#phanCongModal").modal();
	});

	function createOrder(donHangId) {
		$.ajax({
			async: false,
			type : "GET",
			contentType : "application/json",
			url : "http://localhost:8080/order/create/"+donHangId,
			success : function(response) {
				console.log(response.status)
				
				if(response.status === '200'){
					let obj = JSON.parse(JSON.stringify(response.data))
					alert(obj.message_display);
					ajaxPostXacNhanHoanThanh2(donHangId)
					location.href = window.location.href;
				}else{
					let obj = JSON.parse(response.data)
					alert(obj.code_message_value);
				}
	...
	function ajaxPostXacNhanHoanThanh2(donHangId){
		$.ajax({
			async:false,
			type : "POST",
			contentType : "application/json",
			url : "http://localhost:8080/api/don-hang/update?donHangId="+donHangId,
...
//		console.log(donHangId);
		...
		$.get(href, function(donHang) {
			...
			const obj = JSON.parse(donHang.diaChiNhan)
			let formatAddress =
			obj.address + " - " + obj.wardName + " - " +
			obj.districtName + " - " + obj.provinceName
			$('#diaChiNhan').text("Địa chỉ: "+ formatAddress);
